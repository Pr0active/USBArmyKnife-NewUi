...
export default function Marauder() {
  ...
  const handleExecute = async () => {
    if (!command.trim()) {
      showToast('Please enter a Marauder command', 'error')
      return
    }

    setExecuting(true)
    try {
      await api.runMarauder(command.trim())
      setCommandHistory((prev) => [command.trim(), ...prev].slice(0, 10))
      showToast('Marauder command executed successfully', 'success')
      setCommand('')
    } catch (err) {
      showToast(
  ...
  return (
    <div className="px-4 sm:px-0">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">ESP32 Marauder</h1>
        <p className="mt-1 text-sm text-gray-500">
          Execute ESP32 Marauder attack commands. Results will appear in device logs.
        </p>
      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., attack -t rickroll"
